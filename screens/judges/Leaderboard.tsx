...
function isScoringComplete(teamData: any, category: string): boolean {
  ...
  if (
    categoryLower.includes("robo-elem") ||
    categoryLower.includes("robo-junior") ||
    categoryLower.includes("robo-senior")
  ) {
    // Day 1 must be complete (all 3 rounds)
    const day1Complete =
      teamData.day1Round1Score != null &&
      teamData.day1Round2Score != null &&
      teamData.day1Round3Score != null;

    // Day 2 must be complete (all 3 rounds)
    const day2Complete =
      teamData.day2Round1Score != null &&
  ...
  if (
    categoryLower.includes("fi-elem") ||
    categoryLower.includes("fi-junior") ||
    categoryLower.includes("fi-senior")
  ) {
    // All three components must be present
    return (
      teamData.presentationSpirit != null &&
  ...
  // Future Engineering
  if (
    categoryLower.includes("future eng") ||
    categoryLower.includes("future-eng")
  ) {
    // All four rounds must be filled
    return (
      teamData.openScore1 != null &&
    ...
    const fetchTotalTeams = async () => {
      try {
        ...
        if (eventDoc.exists()) {
          ...
          if (
            categoryData &&
            categoryData[judgeCategory] &&
            categoryData[judgeCategory].teams
          ) {
      ...
      (scoresSnap) => {
        ...
        Object.values(teamMap).forEach((team) => {
          ...
          if (
            ...
            const combo1Time =
              (open1 >= open2 ? team.openTime1 : team.openTime2) || "";
            const combo2Time =
              (obstacle1 >= obstacle2
                ? team.obstacleTime1
                : team.obstacleTime2) || "";
            ...
            team.bestTime =
              combinedTimeMs === time1Ms ? combo1Time : combo2Time;
        ...
        const leaderboardArr = Object.values(teamMap)
          .filter(
            (team) => team.bestScore !== undefined && team.bestScore !== null
          )
          .sort((a, b) => {
          ...
          >
            <Text
              style={{
                fontSize: 12,
                fontWeight: "bold",
                color: scoringStatus.includes("finalized")
                  ? "#2d5a3d"
                  : "#b8860b",
                marginBottom: 2,
              }}
      ...
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("AllLeaderboardScreen", {
            ...
            eventTitle: assignedEvents.find((e) => e.id === selectedEvent)
              ?.title,
          })
        }
      >
      ...
      {leaderboard.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            const rankIcons = ["🥇", "🥈", "🥉"];
            const rankDisplay = isTopThree ? rankIcons[index] : `${index + 1}.`;
